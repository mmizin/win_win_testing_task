 ...
 */
test.describe('Landing — guests — adults at maximum', () => {
  ...
  test('UI allows selecting the maximum adults value via the stepper', async ({ page, landing }) => {
    const ceiling = await test.step('Step up until the counter stops (discover cap)', () =>
      withNoServerErrorsOnApiStyleTraffic(page, () => landing.readMaxAdultsCeiling())
    );
    ...
    await test.step('Assert spinbutton matches the discovered ceiling', async () => {
      await expect(landing.adultsSpinbutton()).toHaveValue(String(ceiling));
    });
  });

  test('increasing count past the cap has no effect on the value', async ({ page, landing }) => {
    await withNoServerErrorsOnApiStyleTraffic(page, () => landing.readMaxAdultsCeiling());

    const input = landing.adultsSpinbutton();
    const atMax = await input.inputValue();
    const plus = landing.adultsIncrement();
  ...
  test('a11y attributes expose the current value and the maximum', async ({ page, landing }) => {
    await withNoServerErrorsOnApiStyleTraffic(page, () => landing.readMaxAdultsCeiling());
    const input = landing.adultsSpinbutton();
